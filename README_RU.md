...
<p>
  <strong>Нативный локальный OpenAI-совместимый прокси для Antigravity и Kiro.</strong>
</p>

<p>
  Подключайте аккаунты, запускайте локальный <code>/v1</code> адрес и используйте Antigravity или Kiro из инструментов, которые уже умеют работать с OpenAI API.
</p>
...
KiCk поднимает на вашем устройстве локальный OpenAI-совместимый сервер и пересылает запросы в Antigravity через подключенные Google-аккаунты, а в Kiro через выполненный вход в Kiro. Это нативная оболочка для локального AI-прокси: аккаунты, ключи, журнал, повторы, маршрутизация моделей и запуск одной кнопкой.

| Область | Что есть |
| --- | --- |
| Локальный API | OpenAI-совместимый адрес `http://127.0.0.1:3000/v1` |
| Провайдеры | Antigravity через вход Google, Kiro через вход GitHub или Google |
...
2. Откройте **Аккаунты** и подключите аккаунт Antigravity или Kiro.
3. Для Antigravity укажите идентификатор проекта в `Google Cloud`. Для Kiro выполните вход через GitHub или Google в браузере.
4. Вернитесь на **Главную**, запустите прокси и скопируйте локальный адрес вместе с API-ключом.
5. Используйте их в Antigravity, SillyTavern, другом OpenAI-совместимом клиенте или своем приложении.
...
- Пул аккаунтов Antigravity и Kiro с приоритетами, повторами, временными паузами и фильтрами моделей.
...
- Нет активных аккаунтов: подключите аккаунт Antigravity или Kiro либо включите уже добавленный.
- Истек вход в Google: переподключите аккаунт Antigravity.
